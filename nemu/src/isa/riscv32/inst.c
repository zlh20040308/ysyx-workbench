...
static int decode_exec(Decode *s) {
  ...
  // word_t funct7 = 0, funct3 = 0, opcode = 0;
  // opcode();
  // funct3();
  // funct7();
  // 此时dnpc已经指向下一条指令了
...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */)                   \
  ...
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal, J, R(rd) = s->pc + 4;
          s->dnpc = s->pc + imm);
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr, I,word_t t = s->pc + 4;
  ...
  INSTPAT("0000001 ????? ????? 001 ????? 01100 11", mulh, R,R(rd) =
              BITS(SEXT(BITS(src1, 31, 0), 32) * SEXT(BITS(src2, 31, 0), 32),         63, 32));
  INSTPAT("0000001 ????? ????? 010 ????? 01100 11", mulhsu, R,R(rd) = BITS(SEXT(BITS(src1, 31, 0), 32) * (uint64_t)src2, 63, 32));
