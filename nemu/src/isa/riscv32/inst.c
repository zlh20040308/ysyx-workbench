...
static void decode_operand(Decode *s, int *rd, word_t *src1, word_t *src2,
  ...
  *csr_addr = BITS(i, 31, 20);
  ...
  case TYPE_B:
    ...
    break;
  case TYPE_Z:
    Log("Before src1R, rs1 = %x", rs1);
    src1R();
    Log("After src1R");
    immZ();
    CSR_VAL();
    Log("Is CSR!!!!");
    Log("csr_addr = %x",*csr_addr);
    break;
  }
}
...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */)                   \
  ...
  INSTPAT("??????? ????? ????? 001 ????? 11100 11", csrrw, Z,
          word_t t = csr_val; Log("csr_addr = %x", csr_addr);CSR(csr_addr) = src1; R(rd) = t;);
  ...
  INSTPAT("0000001 ????? ????? 001 ????? 01100 11", mulh, R,
          R(rd) =
              BITS(SEXT(BITS(src1, 31, 0), 32) * SEXT(BITS(src2, 31, 0), 32),
                   63, 32));
  INSTPAT("0000001 ????? ????? 010 ????? 01100 11", mulhsu, R,
          R(rd) = BITS(SEXT(BITS(src1, 31, 0), 32) * (uint64_t)src2, 63, 32));
